...
3. **Validate rule links** — for each violation with a `rule_link`, extract the fragment ID and validate it exists in the target doc:
   ...
   **Violations missing `rule_link` must be recovered or dropped.** Every comment the bot posts must cite a specific best practice rule so developers can understand the basis and push back if needed. If a subagent returns a violation without a `rule_link`, search the subagent's best practice document for a heading that matches the violation's subject. If a matching rule with an `<a id>` anchor exists, add the `rule_link` and post. If no matching rule exists, do NOT post — log: `DROPPED: no rule_link for <file>:<line> — "<draft_comment snippet>"`. General observations that don't map to a documented rule should not be posted as review comments.
