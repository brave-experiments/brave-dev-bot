...
0 3,12 * * * cd $SCRIPT_DIR && git fetch origin && git checkout master && git reset --hard origin/master && $CLAUDE_BIN -p '/add-backlog-to-prd' --allowedTools '$CLAUDE_TOOLS' >> $LOG_DIR/add-backlog-cron.log 2>&1

# Main agent run
0 4,13 * * * cd $SCRIPT_DIR && git fetch origin && git checkout master && git reset --hard origin/master && ./run.sh 3 >> $LOG_DIR/run-cron.log 2>&1

# Review PRs
0 5,8,10,12,14,16,18,20 * * * cd $SCRIPT_DIR && git fetch origin && git checkout master && git reset --hard origin/master && $CLAUDE_BIN -p '/review-prs 1d open auto' --allowedTools '$CLAUDE_TOOLS' >> $LOG_DIR/review-prs-cron.log 2>&1

# Learnable pattern search
0 6 * * * cd $SCRIPT_DIR && git fetch origin && git checkout master && git reset --hard origin/master && $CLAUDE_BIN -p '/learnable-pattern-search --username netzenbot 2d' --allowedTools '$CLAUDE_TOOLS' >> $LOG_DIR/learnable-pattern-search-cron.log 2>&1

# Check Signal messages (every hour, only runs Claude if messages pending)
0 * * * * cd $SCRIPT_DIR && git fetch origin && git checkout master && git reset --hard origin/master && source .envrc && ./scripts/check-signal-messages.sh && $CLAUDE_BIN -p '/check-signal' --allowedTools '$CLAUDE_TOOLS' >> $LOG_DIR/check-signal-cron.log 2>&1
